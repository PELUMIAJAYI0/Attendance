document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        welcomeMessage.textContent = `Welcome, ${user.full_name || user.email}!`;
...
function renderInternDashboard(container) {
    ...
    document.getElementById('clock-in-btn').addEventListener('click', async (e) => {
        ...
        try {
            ...
            const statusMessage = document.getElementById('status-message');
            statusMessage.textContent = 'Clock-in failed. Please try again.';
            statusMessage.style.color = 'var(--error-color)';
            btn.disabled = false;
            btn.textContent = 'Clock In for Today';
        }
        ...
        .then(data => {
            ...
            summaryContainer.innerHTML = `
                <div class="summary-card card">
                    <h3>Present Today</h3>
                    <div class="count">${presentCount} / ${totalCount}</div>
                </div>
                <div class="summary-card card">
                    <h3>On Time</h3>
                ...
                <div class="summary-card card">
                    <h3>Late</h3>
                    <div class="count">${presentCount - onTimeCount}</div>
                </div>
            `;
            ...
            if (data.present && presentCount > 0) {
                ...
                tableBody.innerHTML = `<tr><td colspan="3" style="text-align:center;">No one has clocked in today.</td></tr>`;
            }
        })
        .catch(error => {
